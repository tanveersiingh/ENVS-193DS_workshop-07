...
### Creating a table with model coefficience, 95% confidence intervals, and more

```{r}
# using function from gtsummary package
...
Question: Does elevation (in meters) predict temperature (in $^\circ^$C)?

H~0~: Elevation (m) does not predict temperature ($^\circ^$C).

H~A~: Elevation (m) predicts temperature ($^\circ^$C).
